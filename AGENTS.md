...
1. **No AI / LLM / semantic-search features anywhere.** The owner has
   explicitly opted out. The command palette is plain fuzzy matching plus a
   rule-based keyword→filter map. Do not introduce embeddings, model calls,
   "smart suggestions" generated by an LLM, or anything that talks to an AI
   API from the client. Keep palette logic deterministic.
